...
namespace ChatServer
{
    class Server
    {
        ...
        {
            // 1. 서버 리스너 설정
            int port = 7777; // 기획자가 수정할 수 있도록 포트 번호를 변수화합니다.
        ...
        private static async Task BroadcastMessageAsync(string message, TcpClient sender)
        {
            ...
            // TODO: 전송에 실패한 클라이언트(연결 끊김)들을 메인 리스트에서 제거하는 로직이 필요할 수 있으나,
            // HandleClientAsync의 finally에서도 처리되므로 여기서는 복잡성을 줄이기 위해 생략합니다.
        }
    }
}